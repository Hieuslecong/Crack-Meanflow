...
class ContinuousRTEmbedding(nn.Module):
    ...
    def forward(self,rt):
        r,t=rt[:,0],rt[:,1]; emb=torch.cat([_sinusoidal(t,self.freq_dim),_sinusoidal(r,self.freq_dim)],dim=-1); return self.mlp(emb.to(self.mlp[0].weight.dtype))
...
class CrackMeanFlowModel(nn.Module):
    ...
    def forward(self,x,r,t,y=None,**kwargs):
        ...
        if hasattr(self.unet,'downblocks'):
            ...
            if factor>1 and (x.shape[-1]%factor or x.shape[-2]%factor): raise ValueError(f'input {tuple(x.shape[-2:])} must be divisible by {factor} for this UNet')
