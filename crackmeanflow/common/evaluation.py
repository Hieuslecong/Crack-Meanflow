...
def _aggregate(collected, threshold, include_structural=True, compute_auprc=True):
    ...
    if compute_auprc:
        out['auprc']=binary_average_precision_from_pairs(collected)
        out['metric_aggregation']+='; auprc is exact global pixel average precision from continuous scores'
