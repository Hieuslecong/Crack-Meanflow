...
from crackmeanflow.common.metrics import compute_segmentation_metrics,cldice_score,boundary_f1_score,geometry_centerline_metrics,binary_average_precision_from_pairs
...
def evaluate_geometry_with_frozen_threshold(model,loader,device,rasterizer,threshold,seed=0,max_radius=16.,collect_per_image=False):
    """Streaming one-step geometry evaluation with CPU score retention for exact AUPRC."""
    model.eval();gen=torch.Generator(device='cpu').manual_seed(int(seed));threshold=float(threshold)
    tp=fp=fn=tn=0.;cls=[];bfs=[];geoms=[];per=[];per_pos=[];per_records=[];ap_pairs=[];gt_pos=pred_pos=total_px=0.;empty_gt=empty_gt_fp=0
    for b in loader:
        img=b['crack'].to(device);gt_pm1=b['mask'].to(device);z=torch.randn((img.shape[0],2,img.shape[-2],img.shape[-1]),generator=gen).to(device)
        geom,prob=sample_geometry_one_step(model,z,img,rasterizer);geom=geom.detach().cpu();prob=prob.detach().cpu().float();gt_pm1=gt_pm1.detach().cpu();gt=((gt_pm1+1)*.5);ap_pairs.append((prob,gt));pred=(prob>threshold).float()
