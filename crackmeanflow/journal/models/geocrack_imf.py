...
class GeoCrackIMFModel(nn.Module):
    ...
    def __init__(self,img_size=256,patch=8,size='S',geometry_ch=2,cond_ch=3,local_refine=False,background_init=-0.95):
        ...
        self.cond_patch=nn.Conv2d(64,d,patch,stride=patch); self.state_patch=nn.Conv2d(64,d,patch,stride=patch)
    ...
    def _features(self,z,t,r,image):
        cond_local=self.cond_local(image); state_local=self.state_local(z); cond=self.cond_patch(cond_local); st=self.state_patch(state_local)
    ...
    def clean_predictions(self,z,t,r,image):
        ...
        B=h.shape[0]; g=self.grid; d=h.shape[-1]; glob=h.transpose(1,2).reshape(B,d,g,g)
        ...
        f=self.fuse(torch.cat([cond_local,state_local,glob],dim=1)); shift,scale=self.film(c).chunk(2,-1); f=f*(1+scale[:,:,None,None])+shift[:,:,None,None]
