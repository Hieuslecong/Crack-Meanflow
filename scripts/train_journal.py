...
def _select_checkpoint_metric(cfg,model,loader,device,rasterizer):
    ...
    for seed in seeds:
        if cfg['backbone']=='geocrack_imf':sweep,_=calibrate_geometry_threshold_on_validation(model,loader,device,rasterizer,thresholds,seed,cfg['model'].get('max_radius',16))
        else:sweep,_=calibrate_threshold_on_validation(model,loader,device,crack_meanflow_sampler,thresholds,1,seed,evcfg.get('cfg_scale',1.0))
        sweeps.append(sweep)
...
def main():
    ...
    if option_c:
        ...
        run_class=a.run_class or ('headline' if planned_steps==research_total_steps else 'diagnostic')
        if run_class=='headline' and planned_steps!=research_total_steps:raise RuntimeError('headline Option-C runs must complete the full research scheduler horizon')
        ...
        max_steps=cfg['train'].get('max_optimizer_steps');planned_steps=min(epoch_capacity,int(max_steps)) if max_steps is not None else epoch_capacity;scheduler_total_steps=planned_steps;research_total_steps=planned_steps;run_class='headline';eligibility=_run_class_flags(run_class)
    ...
    fairness={'samples_train':len(train_ds),'batch_size':nominal_b,'grad_accum_steps':ga,'effective_batch_size_nominal':nominal_b*ga,'optimizer_steps_per_epoch':int(steps),'planned_epochs':int(cfg['train']['epochs']),'planned_optimizer_steps':int(planned_steps),'research_scheduler_total_steps':int(research_total_steps),'run_stop_step':int(planned_steps),'run_class':run_class,'nfe':1,'drop_incomplete_accumulation':drop_incomplete_accum,'dropped_microbatches_per_epoch':(len(tr)%ga) if drop_incomplete_accum else 0,'loader_samples_per_epoch':int(loader_samples),'usable_samples_per_epoch':int(usable_samples),'total_omitted_samples_per_epoch':int(max(0,len(train_ds)-usable_samples)),'sample_coverage_fraction_per_epoch':float(usable_samples/max(len(train_ds),1)),'effective_samples_per_full_optimizer_step':nominal_b*ga,'optimizer_recipe':{'lr':float(cfg['train']['lr']),'weight_decay':float(cfg['train']['weight_decay']),'warmup_epochs':int(cfg['train'].get('warmup_epochs',0)),'warmup_optimizer_steps':int(cfg['train'].get('warmup_epochs',0))*int(steps)},'sampling_protocol':cfg['train'].get('sample_balance') or cfg['train'].get('source_balance') or {'enabled':False},'deterministic_requested':bool(cfg['train'].get('deterministic',False)),'checkpoint_selection_seeds':[int(x) for x in cfg.get('eval',{}).get('checkpoint_selection_seeds',[])],'checkpoint_validation_interval_epochs':int(cfg.get('eval',{}).get('checkpoint_validation_interval_epochs',1)),'eval_batch_size':int(cfg.get('eval',{}).get('batch_size',1))}
    ...
    if a.resume:
        ...
        if extra.get('epoch_complete') is False:raise RuntimeError('scientific resume from a partial-epoch budget checkpoint is prohibited; start a new run or choose a budget aligned to full optimizer-accumulation epochs')
