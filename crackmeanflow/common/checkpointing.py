...
def environment_info(): return {'python':sys.version,'platform':platform.platform(),'torch':str(torch.__version__),'cuda':torch.version.cuda,'cudnn':torch.backends.cudnn.version() if torch.backends.cudnn.is_available() else None}
def config_hash(cfg): return hashlib.sha256(json.dumps(cfg,sort_keys=True,separators=(',',':'),default=str).encode()).hexdigest()
...
def save_checkpoint_atomic(path,*,model,ema,optimizer,scheduler,epoch,global_optimizer_step,cfg,best_val_metric,best_val_threshold,split_manifest_hashes,seed,threshold_metric='f1',extra_state=None):
    payload={'model':model.state_dict(),'ema':getattr(ema,'shadow',ema),'optimizer':optimizer.state_dict(),'scheduler':scheduler.state_dict(),'epoch':int(epoch),'global_optimizer_step':int(global_optimizer_step),'cfg':cfg,'config_hash':config_hash(cfg),'git_commit':_git_commit(),'seed':int(seed),'rng_state':capture_rng_state(),'best_val_metric':float(best_val_metric),'best_val_threshold':float(best_val_threshold),'threshold_metric':threshold_metric,'split_manifest_hashes':dict(split_manifest_hashes),'environment':environment_info(),'extra_state':dict(extra_state or {})}; os.makedirs(os.path.dirname(path) or '.',exist_ok=True); tmp=path+'.tmp'; torch.save(payload,tmp); os.replace(tmp,path)
...
def load_checkpoint(path,model,optimizer=None,scheduler=None,map_location='cpu'):
    ...
    if optimizer is not None and 'optimizer' in ck: optimizer.load_state_dict(ck['optimizer']); _optimizer_to(optimizer,next(model.parameters()).device)
