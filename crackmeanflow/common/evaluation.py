...
def _collect(model,loader,device,sampler,num_steps,seed,cfg_scale=1.0):
    ...
    for batch in loader:
        img=batch['crack'].to(device); gt_pm1=batch['mask'].to(device); z=torch.randn(gt_pm1.shape,generator=gen).to(device); pred_pm1,_=sampler(model,z,img,num_steps=num_steps,cfg_scale=cfg_scale,clamp=False); outs.append((pred_pm1.detach(),((gt_pm1+1)*.5).detach()))
    return outs
def _aggregate(collected,threshold):
    ...
    else: pr=tp/max(tp+fp,1e-12); re=tp/max(tp+fn,1e-12); f1=2*pr*re/max(pr+re,1e-12); iou=tp/max(tp+fp+fn,1e-12)
...
def calibrate_threshold_on_validation(model,loader,device,sampler,thresholds,num_steps=1,seed=0,cfg_scale=1.0):
    coll=_collect(model,loader,device,sampler,num_steps,seed,cfg_scale); rows={float(t):_aggregate(coll,float(t)) for t in thresholds}; return rows,max(rows,key=lambda t:rows[t]['f1'])
def evaluate_with_threshold(model,loader,device,sampler,threshold,num_steps=1,seed=0,cfg_scale=1.0): return _aggregate(_collect(model,loader,device,sampler,num_steps,seed,cfg_scale),float(threshold))
