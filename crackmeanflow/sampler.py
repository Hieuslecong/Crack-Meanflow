...
def crack_meanflow_sampler(model,z,crack_image,num_steps=1,cfg_scale=1.0,clamp=False):
    ...
    def step(x_cur,r_val,t_val):
        ...
        if do_cfg:
            xx=torch.cat([x_cur,x_cur],0);rr=torch.cat([r,r],0);tt=torch.cat([t,t],0);yy=torch.cat([crack_image,torch.zeros_like(crack_image)],0);u_c,u_u=torch.chunk(model(xx,rr,tt,y=yy),2,0);u=u_u+cfg_scale*(u_c-u_u)
