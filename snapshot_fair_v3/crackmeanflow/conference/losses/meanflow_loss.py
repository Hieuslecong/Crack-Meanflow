"""True conditional MeanFlow loss for the Conference track."""
...
class ConferenceMeanFlowLoss(nn.Module):
    ...
    def _stage(self):
        interval_prob=self.base_interval; max_gap=1.0; boundary_prob=self.boundary_prob; tc=self.time_curriculum or {}
        if tc.get('enabled'):
        ...
        if max_gap < 1.0: boundary_prob=0.0
    ...
    def _sample_r_t(self,b,device,return_stage=False,sample_offset=None):
        ...
        if boundary_prob<=0: boundary=torch.zeros(b,device=device,dtype=torch.bool)
            ...
            idx=torch.arange(int(sample_offset),int(sample_offset)+b,device=device,dtype=torch.float64); p=float(boundary_prob); boundary=torch.floor((idx+1.0)*p)>torch.floor(idx*p)
        else: boundary=torch.rand(b,device=device)<boundary_prob
        r=torch.where(boundary,torch.zeros_like(r),r); t=torch.where(boundary,torch.ones_like(t),t)
        stage={'interval_prob':interval_prob,'max_gap':max_gap,'boundary_prob':boundary_prob,'boundary_sampling':self.boundary_sampling,'boundary_count':int(boundary.sum().item())}; return (r,t,stage) if return_stage else (r,t)
    @staticmethod
    ...
    def forward(self,model,x0,model_kwargs=None):
        ...
        sample_offset=kw.get('sample_offset',None); r,t,stage=self._sample_r_t(b,device,return_stage=True,sample_offset=sample_offset); eps=torch.randn_like(x0); z=(1-_b(t))*x0+_b(t)*eps; v=eps-x0
        u=model(z,r,t,y=y); base=model.module if hasattr(model,'module') else model; seg_logits=base.get_seg_logits()
        ...
        target=(v-_b(t-r)*dudt).detach(); sq=(u-target).pow(2).flatten(1).mean(1); weight=(sq.detach()+self.adaptive_eps).pow(self.adaptive_p); mf=(sq/weight).mean(); total=mf.clone()
        endpoint=x0.new_tensor(0.); thin=x0.new_tensor(0.); seg=x0.new_tensor(0.); at_zero=(r==0)
