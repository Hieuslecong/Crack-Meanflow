...
class GeometryRasterizer(nn.Module):
    ...
    def _disk(radius:int,device,dtype):
        ...
        return ((xx*xx+yy*yy)<=radius*radius).to(dtype)[None,None]
    def forward_fields(self,center_prob,radius):
        ...
        for k in range(self.bins):
            r=int(round(float(centers[0,k,0,0].detach().cpu()))); src=center_prob*weights[:,k:k+1]; ker=self._disk(r,src.device,src.dtype)
            spread=1-torch.exp(-self.sharpness*F.conv2d(src,ker,padding=r)); union=1-(1-union)*(1-spread.clamp(0,1))
