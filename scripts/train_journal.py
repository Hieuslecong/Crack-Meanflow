...
def main():
    ...
    run_started=time.time();cfg=yaml.safe_load(open(a.config));option_c=cfg.get('protocol_id')=='OPTION_C_V1'
    if option_c and a.max_optimizer_steps is not None:raise ValueError('OPTION_C_V1 forbids --max-optimizer-steps; use --research-total-steps and --stop-after-step')
    if (not option_c) and (a.research_total_steps is not None or a.stop_after_step is not None or a.run_class is not None):raise ValueError('Option-C budget/run-class flags require protocol_id=OPTION_C_V1')
    ...
    if option_c:
        ...
        milestone_steps=milestone_steps_from_config(cfg['train']);design_lock_sha=option_c_design_lock_sha256()
        ...
        max_steps=cfg['train'].get('max_optimizer_steps');planned_steps=min(epoch_capacity,int(max_steps)) if max_steps is not None else epoch_capacity;scheduler_total_steps=planned_steps;research_total_steps=planned_steps;run_class='headline';eligibility=_run_class_flags(run_class);milestone_steps=();design_lock_sha=None
    ...
    for ep in range(start_epoch,cfg['train']['epochs']):
        ...
        for i,b in enumerate(tr):
            ...
            if (i+1)%cfg['train']['grad_accum_steps']==0 or i+1==len(tr):
                ...
                if option_c and gs in milestone_steps:
    ...
    milestone_artifacts=[os.path.join('checkpoints',milestone_checkpoint_name(s)) for s in milestone_steps if int(s)<=int(planned_steps)]
