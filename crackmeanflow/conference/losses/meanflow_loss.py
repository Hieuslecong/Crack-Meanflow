...
class ConferenceMeanFlowLoss(nn.Module):
    ...
    def _sample_r_t(self,b,device,return_stage=False,sample_offset=None):
        ...
        r=torch.where(torch.rand(b,device=device)>=interval_prob,t,r)
        ...
        r=torch.where(boundary,torch.zeros_like(r),r); t=torch.where(boundary,torch.ones_like(t),t); st={'interval_prob':interval_prob,'max_gap':max_gap,'boundary_prob':boundary_prob,'boundary_sampling':self.boundary_sampling,'boundary_count':int(boundary.sum())}; return (r,t,st) if return_stage else (r,t)
    @staticmethod
    ...
    def forward(self,model,x0,model_kwargs=None):
        ...
        r,t,stage=self._sample_r_t(b,device,True,kw.get('sample_offset')); eps=torch.randn_like(x0); z=(1-_b(t))*x0+_b(t)*eps; v=eps-x0; u=model(z,r,t,y=y); base=model.module if hasattr(model,'module') else model; seg_logits=base.get_seg_logits()
        ...
        target=(v-_b(t-r)*dudt).detach(); sq=(u-target).pow(2).flatten(1).mean(1); weight=(sq.detach()+self.adaptive_eps).pow(self.adaptive_p); mf=(sq/weight).mean(); total=mf.clone(); endpoint=x0.new_tensor(0.); thin=x0.new_tensor(0.); seg=x0.new_tensor(0.)
        at_zero=(r==0)
        ...
        if self.seg_loss_weight>0 and seg_logits is not None: seg=F.binary_cross_entropy_with_logits(seg_logits.float(),(x0+1)*.5); total+=self.seg_loss_weight*seg
