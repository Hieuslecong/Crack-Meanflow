...
def geometry_consistency_components(g_long,g_short,rasterizer,center_weight=1.0,radius_weight=.5,mask_weight=.5):
    """Compare two endpoint states in geometry and rendered-mask space."""
    ...
    total=float(center_weight)*center+float(radius_weight)*radius/max(float(rasterizer.max_radius),1e-6)+float(mask_weight)*mask
    return total,{'gic_center':center,'gic_radius':radius,'gic_mask':mask}

def geometry_interval_consistency(model,z,r,t,image,rasterizer,center_weight=1.0,radius_weight=.5,mask_weight=.5):
    ...
    return geometry_consistency_components(g_long,g_short,rasterizer,center_weight=center_weight,radius_weight=radius_weight,mask_weight=mask_weight)
