...
def geometry_interval_consistency(model,z,r,t,image,rasterizer,mask_weight=.5,radius_weight=.5):
    ...
    Compare direct t->0 transport with the composition t->s->0. Both compared
    states are clean-geometry endpoints; intermediate noisy states are never
    rasterized or interpreted as physical crack geometry.
    """
    del r
    ...
    if rasterizer.representation == 'centerline_edt': support=torch.maximum(mask_long,mask_short).detach()
    else: support=torch.maximum(c_long,c_short).detach()
