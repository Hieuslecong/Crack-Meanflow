...
def mask_to_geometry_state(mask_pm1: torch.Tensor, max_radius: float=16.0, representation: str='centerline_radius', distance_encoding: str='linear'):
    ...
    channel 0: centerline occupancy mapped to [-1,1]
    channel 1: radius/max_radius mapped to [-1,1], radius is zero off centerline.
    Returns (state, radius_valid_mask).
    """
    mask01=(mask_pm1.detach().float()>0).cpu().numpy()
    states=[]; valids=[]
    for sample in mask01:
        m=sample[0].astype(bool)
        c=skeletonize(m)
        ...
        if representation == 'centerline_radius':
            ...
            if distance_encoding == 'sqrt':
                field=np.sqrt(field)
            elif distance_encoding == 'log1p':
                field=np.log1p(d.clip(0,float(max_radius)))/np.log1p(float(max_radius))
            elif distance_encoding != 'linear':
                raise ValueError(f'unknown distance_encoding={distance_encoding!r}')
        else:
            raise ValueError(f'unknown geometry representation={representation!r}')
    ...
    state=torch.stack(states).to(mask_pm1.device,dtype=torch.float32)
    valid=torch.stack(valids).to(mask_pm1.device,dtype=torch.float32)
    return state,valid
...
def geometry_state_to_fields(state_pm1: torch.Tensor, max_radius: float=16.0, representation: str='centerline_radius', distance_encoding: str='linear'):
    center=((state_pm1[:,0:1]+1)*.5).clamp(0,1)
    q=((state_pm1[:,1:2]+1)*.5).clamp(0,1)
