...
def _tracked_worktree_clean() -> tuple[bool, dict]:
    ...
    return bool(unstaged and staged), {"unstaged_clean": bool(unstaged), "staged_clean": bool(staged)}
...
def main() -> None:
    ...
    ap.add_argument("--allow-tracked-dirty", action="store_true")
    args = ap.parse_args()
    ...
    clean, clean_detail = _tracked_worktree_clean()
    if not clean and not args.allow_tracked_dirty:
        raise RuntimeError("tracked worktree is dirty; paper-v3 provenance gate refuses to proceed")

    provenance = {
        "branch": _git("branch", "--show-current"),
        "commit": _git("rev-parse", "HEAD"),
        "tracked_worktree_clean": clean,
        "tracked_worktree_detail": clean_detail,
    ...
    for arm, cfg_path in protocol["primary_arms"].items():
        ...
        arms[arm] = {
            ...
            "config_semantic_sha256": config_hash(cfg),
