...
def mask_to_geometry_state(mask_pm1,max_radius=16.0,representation='centerline_radius',distance_encoding='linear'):
    ...
    for sample in mask01:
        ...
        if representation=='centerline_radius': field=(d*c.astype(np.float32)).clip(0,float(max_radius))/float(max_radius)
        ...
        s=np.stack([c.astype(np.float32)*2-1,field*2-1],axis=0); states.append(torch.from_numpy(s));valids.append(torch.from_numpy(c.astype(np.float32))[None])
...
def geometry_state_to_fields(state_pm1,max_radius=16.0,representation='centerline_radius',distance_encoding='linear'):
