...
from crackmeanflow.common import (
    PairedCrackDataset,discover_required_splits,manifest_hash,audit_group_integrity,
    calibrate_threshold_on_validation,make_warmup_cosine_scheduler,optimizer_steps_per_epoch,
    save_checkpoint_atomic,load_checkpoint,restore_rng_state,config_hash,EMA,
    discover_normal_images,append_normal_negatives,source_balancing_weights,EpochWeightedRandomSampler,
)
...
def build_track(cfg,device):
    if cfg['backbone']=='geocrack_imf':
        model=build_geocrack_imf(cfg['model']).to(device); rast=GeometryRasterizer(cfg['model'].get('max_radius',16),cfg['model'].get('radius_bins',8),representation=cfg['model'].get('representation','centerline_radius'),distance_encoding=cfg['model'].get('distance_encoding','linear')).to(device); lossfn=ImprovedMeanFlowGeometryLoss(**cfg['loss'],max_radius=cfg['model'].get('max_radius',16),rasterizer=rast); return model,rast,lossfn
    if cfg['backbone']=='sit_imf_mask':
        model=MaskIMFSiTModel(cfg['model']['img_size'],cfg['model']['patch'],cfg['model']['size'],cfg['model'].get('background_init',-0.95)).to(device); return model,None,ImprovedMeanFlowStateLoss(**cfg['loss'])
    if cfg['backbone']=='hybrid_imf_mask':
        model=HybridMaskIMFModel(cfg['model']['img_size'],cfg['model']['patch'],cfg['model'].get('size','S'),cfg['model'].get('background_init',-0.95)).to(device); return model,None,ImprovedMeanFlowStateLoss(**cfg['loss'])
    if cfg['backbone']=='sit_mf':
        sit=build_sit(cfg['model']['img_size'],cfg['model']['patch'],cfg['model']['size'],cfg['model'].get('in_ch',1),cfg['model'].get('cond_ch',3)); model=CrackMeanFlowModel(sit,T=500).to(device); return model,None,ConferenceMeanFlowLoss(**cfg['loss'])
    raise RuntimeError(f"unsupported journal ablation backbone={cfg['backbone']!r}")

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--config',required=True); ap.add_argument('--data',required=True); ap.add_argument('--out',default=None); ap.add_argument('--resume',default=None); ap.add_argument('--allow-config-change',action='store_true'); ap.add_argument('--group-regex',default=None); a=ap.parse_args()
    cfg=yaml.safe_load(open(a.config));
    if cfg.get('track') not in {'journal','journal_ablation'}: raise RuntimeError(f"invalid Journal track={cfg.get('track')!r}")
    seed=cfg['train'].get('seed',42); seed_all(seed,cfg['train'].get('deterministic',False)); device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    base_sp=discover_required_splits(a.data)
    ...
    tr=_train_loader(train_ds,sp['train'],cfg,seed); va=DataLoader(val_ds,batch_size=1,shuffle=False,num_workers=cfg['train'].get('num_workers',0))
    ...
    if a.resume:
        ...
        if not a.allow_config_change and ck.get('config_hash') and ck['config_hash']!=config_hash(cfg): raise RuntimeError('resume config differs from checkpoint')
        ema=EMA(model,cfg['train']['ema_decay'],ck.get('ema')); best=float(ck.get('best_val_metric',-1)); best_th=float(ck.get('best_val_threshold',.5)); gs=int(ck.get('global_optimizer_step',0)); start_epoch=int(ck['epoch'])+1; global_sample_offset=int((ck.get('extra_state') or {}).get('global_sample_offset',0)); restore_rng_state(ck.get('rng_state'))
    ...
    for ep in range(start_epoch,cfg['train']['epochs']):
        ...
        model.train(); opt.zero_grad(set_to_none=True); ls=[]; t0=time.time(); fm_samples=0; gic_samples=0; seen_samples=0
        ...
        ema.apply(model)
        try:
            if cfg['backbone']=='geocrack_imf': sweep,th=calibrate_geometry_threshold_on_validation(model,va,device,rast,cfg['eval'].get('thresholds',[.3,.4,.5,.6,.7]),cfg['eval']['eval_seeds'][0],cfg['model'].get('max_radius',16)); ev=sweep[th]
        ...
        score=ev['f1']; row={'epoch':ep,'loss':float(np.mean(ls)),'val_f1':score,'val_threshold':float(th),'optimizer_step':gs,'lr':opt.param_groups[0]['lr'],'seconds':time.time()-t0,'fm_samples':fm_samples,'gic_samples':gic_samples,'seen_samples':seen_samples,'fm_fraction':fm_samples/max(seen_samples,1),'gic_fraction':gic_samples/max(seen_samples,1)}; hist.append(row); print(row); improved=score>best
        ...
        json.dump({'history':hist,'best_val_f1':best,'best_val_threshold':best_th},open(os.path.join(out,'history.json'),'w'),indent=2)
