...
def compute_segmentation_metrics(pred_binary,mask_gt,eps=1e-7):
    ...
    if (g.sum()==0) and (p.sum()==0): return {k:1.0 for k in ['iou','dice','f1','precision','recall','accuracy']}|{'tp':0.,'fp':0.,'fn':0.,'tn':_scalar(tn)}
...
def _boundary(x): x=_bin(x); er=1-F.max_pool2d(1-x,3,1,1); return (x-er).clamp(0,1)
...
def skeleton_length_px(binary):
    ...
    for a in sk[:,0]: total+=float((a[:,:-1]*a[:,1:]).sum())+float((a[:-1,:]*a[1:,:]).sum())+2**.5*float((a[:-1,:-1]*a[1:,1:]).sum())+2**.5*float((a[:-1,1:]*a[1:,:-1]).sum())
    return total
def geometry_centerline_metrics(pred_center_prob,gt_center_binary,pred_radius=None,gt_radius=None,max_radius=16.0):
    ...
    for bi in range(p.shape[0]):
        pyx=np.argwhere(p[bi,0].detach().cpu().numpy()>0.5); gyx=np.argwhere(g[bi,0].detach().cpu().numpy()>0.5); diag=float((p.shape[-2]**2+p.shape[-1]**2)**.5)
