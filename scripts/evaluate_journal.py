...
def _validate_sit_v1_target_access(protocol_id,eligibility_class,dataset_name):
    if protocol_id!='CRACKMEANFLOW_SIT_V1' or eligibility_class!='screen':
        return
    allowed={'GAPS384','OmniCrack30k_repartitioned_v013_holdout'}
    if dataset_name not in allowed:
        raise RuntimeError(f'SiT-V1 screen-stage target firewall rejected dataset={dataset_name!r}; allowed development OOD={sorted(allowed)}')
...
def main():
    ...
    required_metrics=list(CORE_KEYS)
    if bb=='geocrack_imf': required_metrics.extend(['centerline_assd_px','edt_radius_mae_px','skeleton_length_rel_error'])
    missing_metrics=[key for key in required_metrics if key not in out or not isinstance(out.get(key),dict) or out[key].get('mean') is None]
    if missing_metrics: raise RuntimeError(f'evaluation artifact is missing required metric(s): {", ".join(missing_metrics)}')
