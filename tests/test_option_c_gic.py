...
def test_explicit_gic_component_weights_control_total():
    rast=GeometryRasterizer(max_radius=4,bins=5,temperature=.25,representation='centerline_radius');a=_state(radius=1.);b=_state((9,8),3.)
    total,p=geometry_consistency_components(a,b,rast,center_weight=0.,radius_weight=0.,mask_weight=1.)
    assert torch.allclose(total,p['gic_mask'])
