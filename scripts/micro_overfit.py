...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--config',required=True);ap.add_argument('--data',required=True);ap.add_argument('--subset',type=int,default=8);ap.add_argument('--steps',type=int,default=200);ap.add_argument('--image-size',type=int,default=128);ap.add_argument('--batch-size',type=int,default=1);ap.add_argument('--runtime-batch-size',type=int,default=None);ap.add_argument('--runtime-grad-accum-steps',type=int,default=1);ap.add_argument('--seed',type=int,default=0);ap.add_argument('--lr',type=float,default=None);ap.add_argument('--out',default=None);ap.add_argument('--max-loss-ratio',type=float,default=.70);ap.add_argument('--min-f1-gain',type=float,default=.15);ap.add_argument('--min-final-f1',type=float,default=None);a=ap.parse_args()
    if a.runtime_grad_accum_steps < 1:
        ap.error('--runtime-grad-accum-steps must be >= 1')
    runtime_batch_size=a.runtime_batch_size or a.batch_size
    seed_all(a.seed);device=torch.device('cuda' if torch.cuda.is_available() else 'cpu');cfg=copy.deepcopy(yaml.safe_load(open(a.config)));cfg['model']['img_size']=a.image_size;cfg['train']['batch_size']=runtime_batch_size;cfg['train']['drop_last']=False;cfg['train']['seed']=a.seed
    splits=discover_required_splits(a.data);pairs=sorted(splits['train'])[:max(1,a.subset)];base=PairedCrackDataset(pairs,a.image_size,augment=False,photometric=False,mask_resize_mode=cfg['train'].get('mask_resize_mode','nearest'));ds=GeometryDataset(base,cfg['model'].get('max_radius',16),cfg['model'].get('representation','centerline_radius'),cfg['model'].get('distance_encoding','linear')) if cfg['backbone']=='geocrack_imf' else base;loader=DataLoader(ds,batch_size=runtime_batch_size,shuffle=True,drop_last=False);eval_loader=DataLoader(ds,batch_size=1,shuffle=False);model,rast,lossfn=build_track(cfg,device);opt=torch.optim.AdamW(model.parameters(),lr=a.lr or cfg['train']['lr'],weight_decay=cfg['train']['weight_decay']);initial,initial_th=evaluate(model,rast,cfg,eval_loader,device,a.seed);losses=[];offset=0;it=itertools.cycle(loader);model.train()
    for step in range(a.steps):
        opt.zero_grad(set_to_none=True);step_losses=[]
        for _ in range(a.runtime_grad_accum_steps):
            b=next(it);img=b['crack'].to(device);bs=img.shape[0]
            ...
            offset+=bs;(loss/a.runtime_grad_accum_steps).backward();step_losses.append(float(loss.detach()))
        torch.nn.utils.clip_grad_norm_(model.parameters(),cfg['train'].get('max_grad_norm',1.0));opt.step();losses.append(float(np.mean(step_losses)))
